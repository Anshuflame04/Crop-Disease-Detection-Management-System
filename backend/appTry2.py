...
models = {
    ...
    'apple': tf.keras.models.load_model(r"A:\PROJECTS\DiseaseDetection\TryReact\plant-app1\backend\model\apple_model_v1.keras")
}

disease_details = {
    'tomato': json.load(open(r'A:\WebDev\plant_app\plant-app1\backend\data\TomatoDetails.json')),
    'potato': json.load(open(r'A:\WebDev\plant_app\plant-app1\backend\data\PotatoDetails.json')),
    'rice': json.load(open(r'A:\WebDev\plant_app\plant-app1\backend\data\RiceDetails.json')),
    'wheat': json.load(open(r'A:\WebDev\plant_app\plant-app1\backend\data\TomatoDetails.json')),
    'apple': json.load(open(r'A:\WebDev\plant_app\plant-app1\backend\data\AppleDetails.json'))
}
...
async def predict(crop_name: str, file: UploadFile = File(...)):
    ...
    try:
        ...
        disease_info = disease_dict.get(class_name, {
            ...
            'organic_pesticides': 'No organic pesticides available.',
            'natural_fertilizers': 'No natural fertilizers available.'
        })
